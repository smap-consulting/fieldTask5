...
class GeoPolyFragment @JvmOverloads constructor(
    ...
    fun initMap(map: MapFragment, binding: GeopolyLayoutBinding) {
        ...
        // smap - "history-map": outline polygons captured by earlier repeat instances
        ...
        if (!map.hasCenter()) {
            // smap - frame the previous polygons too, not just this answer's points
            val allPoints = viewModel.points.value + previousPolygons.flatten()
