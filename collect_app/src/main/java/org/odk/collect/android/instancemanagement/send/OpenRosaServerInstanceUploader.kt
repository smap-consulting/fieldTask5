...
class OpenRosaServerInstanceUploader(
    ...
    private var acceptContentLength = DEFAULT_CONTENT_LENGTH
    ...
    override fun uploadOneSubmission(
        ...
        val submissionUri = resolveSubmissionUri(urlString, webCredentialsUtils)
        val contentLength = acceptContentLength
        ...
        try {
            ...
            // smap - status, locationTrigger, surveyNotes and assignmentId travel with the submission
            val postResult = httpInterface.uploadSubmissionAndFiles(
                    ...
                    instance.status,
                    instance.locationTrigger,
                    instance.surveyNotes,
    ...
    /**
     * smap - Issues the OpenRosa HEAD request for a submission URL to confirm the endpoint,
     * learn the accepted content length and resolve any redirect. The result is cached in
     * [uriRemap] so it is issued at most once per URL per batch.
     *
     * Extracted from [uploadOneSubmission] so it can also be used as a pre-batch
     * authentication probe. No instance file is read before this succeeds.
     */
    private fun resolveSubmissionUri(urlString: String, webCredentialsUtils: WebCredentialsUtils): Uri {
        ...
        if (uriRemap.containsKey(submissionUri)) {
            val remapped = uriRemap[submissionUri]!!
        ...
        try {
            ...
            if (responseHeaders.containsHeader(OpenRosaConstants.ACCEPT_CONTENT_LENGTH_HEADER)) {
                ...
                try {
                    acceptContentLength = contentLengthString!!.toLong()
                } catch (e: Exception) {
    ...
    // smap - probe the endpoint once for a batch rather than once per instance
    override fun checkSubmissionAuth(projectId: String, instance: Instance, deviceId: String?, overrideURL: String?) {
        val projectDependencyModule = projectDependencyFactory.create(projectId)
        val unprotectedSettings = projectDependencyModule.generalSettings
        val webCredentialsUtils = WebCredentialsUtils(unprotectedSettings)

        resolveSubmissionUri(
            getUrlToSubmitTo(instance, deviceId, overrideURL, unprotectedSettings),
            webCredentialsUtils
        )
    }
    ...
    private fun markSubmissionComplete(instance: Instance, instancesRepository: InstancesRepository) {
        ...
        val instanceDatabaseUri = Uri.withAppendedPath(
        ...
        if (!instance.isCase) { // smap - Regular tasks/forms
            ...
            contentValues.put(DatabaseInstanceColumns.T_TASK_STATUS, Utilities.STATUS_T_ACCEPTED)
        }

        Collect.getInstance().contentResolver.update(instanceDatabaseUri, contentValues, null, null)
    }
    ...
    companion object {
        ...
        private const val DEFAULT_CONTENT_LENGTH = 10_000_000L // smap
