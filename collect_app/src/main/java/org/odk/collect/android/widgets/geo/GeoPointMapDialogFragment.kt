...
import org.odk.collect.android.widgets.utilities.HistoryMapUtils
...
class GeoPointMapDialogFragment(
    ...
    override fun onCreateFragment(
        ...
        return GeoPointMapFragment(
            ...
            getPreviousPoints(prompt) // smap - "history-map"
        )
    }

    // smap - "history-map": locations captured by earlier instances of this repeat
    private fun getPreviousPoints(prompt: FormEntryPrompt): List<MapPoint> {
        return HistoryMapUtils.getPreviousGeometryValues(getFormController(), prompt)
            .mapNotNull { GeoPolyUtils.parseGeometry(it).firstOrNull() }
    }
